...
def read(filename, myDisk):
	#1.0 - Open .img file and read his structure.
	f = open (filename, 'rb')
	allText = f.read()
	f.close()

	#After the reading operation, all text will inside an immutable array of bytes. 
	#To work with it, I need to read/write so I need to convert it to a bytearray.
	myDisk.content = bytearray(allText)

############################################# USEFUL DISK INFORMATIONS #############################################
def info(filename, myDisk):
	...
	myDisk.endBOOT = myDisk.sizeBOOT - 1 #-1 because first sector is 0. So i.e., if we have 8 sectors for boot block it will finish at sector 7.
	...
	myDisk.sizeFATinBYTES = myDisk.sizeFAT * 512
	myDisk.startFAT = myDisk.endBOOT + 1
	print("Start Sector: ", myDisk.startFAT)
	myDisk.endFAT = (myDisk.sizeFAT * myDisk.nrFAT) + myDisk.startFAT
...
#VERY IMPORTANT NOTE: IF I HAVE TO CHECK FOR EXAMPLE SECTOR 9, IT IS EQUAL TO 10, BECAUSE SECTORS STARTS AT 0. AND IF I HAVE TO CHECK LAST ELEMENT OF SECTOR 0, IT IS 511 BECAUSE
#THE "content" array that contains all my bytes starts at position 0!
##########################################################################################

#5.0 Check free space 
def freeSpace(myDisk):
	print("\n--- Checking Free Space ---")
	print("Loading...")

	for x in range((((myDisk.startFAT+1)* 512) - 512), (myDisk.startFAT + 1 + myDisk.sizeFAT)* 512 - 512 ,2):
		if myDisk.content[x] == 0 and myDisk.content[x+1] == 0:
			myDisk.freeSpace += 1
	...
	#Disk must have 15% free space to be defragmented. Let's calculate it.
	Percentage15 = myDisk.totalSECTORS * 15 // 100 
	myDisk.SpacePercentage = ((myDisk.freeSpace * myDisk.SectorPerCluster) * 100) // myDisk.totalSECTORS
	print("Free Space in percentage: ", myDisk.SpacePercentage ,"%")
	if myDisk.SpacePercentage > 15:
